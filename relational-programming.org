...
It takes two goals---~g1~ and ~g2~---and returns the stream that
results from merging ~(g1 s/c)~ and ~(g2 s/c)~.
...
#+begin_src scheme -n :scheme chez :exports both :results output verbatim
  ...
  (define stream ((call/fresh (lambda (x) (fives x))) empty-state))
