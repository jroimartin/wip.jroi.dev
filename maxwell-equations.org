...
The reason is that it uses [[https://en.wikipedia.org/wiki/M-expression][M-expressions]] instead of [[https://en.wikipedia.org/wiki/S-expression][S-expressions]].

As the Lisp 1.5 programmers manual explains on page 1:
...
The following is a the complete [[https://racket-lang.org/][Racket]] implementation ([[file:maxwell-equations/evalquote.rkt][evalquote.rkt]]):

#+include: "maxwell-equations/evalquote.rkt" src scheme -n

And the same code after commenting every single bit
([[file:maxwell-equations/evalquote-commented.rkt][evalquote-commented.rkt]]):
...
intance, the following two lines patch allows to call procedures
...
If you like this topic, then stop everything are doing and go watch
the wonderful talk "[[https://www.youtube.com/watch?v=OyfBQmvr2Hc][The Most Beautiful Program Ever Written]]" by
William Byrd.
